...
**UnderwritingRequest W-9 (2026-08-07):** Deal Room **Underwriting requests** panel (selected MID) — agent sends W-9 invite (email/SMS/both) via `manageUnderwritingRequest`; merchant signs at `/uw/:token` via `completeUnderwritingRequest` (in-house e-sign + stamped IRS PDF, not BoldSign). Signed PDF: Download or **Send to Elavon** from underwriting@ (Gmail **`gmail.send`**). Document To presets: `FulSerCenter@elavon.com` / `MSPFulSer@elavon.com`, or type an assigned Elavon rep; optional env `UNDERWRITING_ELAVON_DOCS_TO` overrides the default To. **Never** send docs to `ApplicationStatus@elavon.com` (status inquiry only — use Deal Room **Request status**). One active unsigned request per MID+`w9`; does **not** replace checklist **Request document**. Entity `UnderwritingRequest` must be republished in Base44. Spec: `docs/superpowers/specs/2026-08-07-underwriting-w9-request-design.md`; ops: `docs/underwriting-inbox.md` § W-9.
...
**Locations 1∶1 combined panel (2026-07-16; widened 2026-07-30 #12):** when a **location has exactly one MID**, `OnboardingLocations` shows one store card (name/address + processing fields) instead of nested Location+MID boxes — regardless of entity count. When a location has **2+ MIDs**, the location header is **address-only** (organizer); named MidCards underneath. Full-app 1 entity × ≤1 location still uses store-first + Legal entity chrome (`simpleMode`). No schema or save-path changes.
