...
Remaining un-migrated: **ProgressTracker** (top-nav stepper — still amber-scale, though amber now = brand gold via the Tailwind override, so it's already on-brand; migrate to explicit `cb-*` when convenient). All four merchant-facing onboarding step pages (Locations, Banking, Welcome Hub, Verification) are now on the token system.
