...
| `importMSPPortfolio` | MSP **Merchants** API → Merchant Center hierarchy: **Owner (email)** → **Legal Entity** (`federal_tax_id` / corporate_name) → **MID**. Optional form/signatures enrichment when appNo known. Stable profile `msp-{tin}` / `msp-corp-{slug}`. Admin-only. `{ dryRun: true }` / `{ confirmLive: true }`. No HubSpot. UI: `/admin/center/sync-msp`. |
