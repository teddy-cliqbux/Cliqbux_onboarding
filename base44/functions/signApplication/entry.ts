...
function pricingNotReadyMessage(profile: any, tierKey: string): string | null {
  ...
  if (CUSTOM_PRICING_TIERS.includes(tierKey) &&
    ...
    return (
      ...
      `application can be created or filled. Set them in Admin → Applications → Pricing or on the HubSpot deal ` +
      `(custom_markup_percentage + custom_per_tx_fee + custom_auth_per_card).`
    );
  }
  return null;
}
...
function buildFormPayload(
  ...
  // GUARD (2026-07-06 / 2026-07-14): Custom Flat Rate / Custom Interchange Plus need
  // negotiated fees. Legacy STANDARD (and friends) are treated as "pricing not set" —
  // agents must Save Pricing in Applications (Cash Discount or custom), not HubSpot Sync alone.
...
Deno.serve(async (req) => {
  try {
    ...
    // Fail fast before MSPWare draft create. Legacy STANDARD counts as unconfigured.
    {
