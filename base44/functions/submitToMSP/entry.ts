...
Deno.serve(async (req) => {
  try {
    ...
    // ── Fetch merchant data ────────────────────────────────────────────────────
    const [profiles, allMerchantMIDs, allLocs, signersAll] = await Promise.all([
    ...
    try {
      ...
      console.warn('[submitToMSP] includedSignerIds filter failed (using all signers):', selErr?.message);
    }
