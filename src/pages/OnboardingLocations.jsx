...
import { Plus, ArrowRight, Loader2, Store, Landmark, Trash2, CheckCircle2, AlertCircle, Pencil, Check, MapPin } from 'lucide-react';
...
export default function OnboardingLocations({ profile, onContinue }) {
  ...
  if (loading) return <div className="flex flex-col items-center justify-center py-24 gap-3"><Loader2 className="w-8 h-8 text-gray-400 animate-spin" /><p className="text-sm text-gray-500">Loading your locations...</p></div>;
    ...
    <div className="flex flex-col">
      ...
      <div className="px-8 pt-8 pb-6 border-b border-gray-100">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Enter each business location with address verification, legal entity assignment, and settlement banking.</p>
        </div>
      </div>
        ...
        {locs.length === 0 ? (
          ...
          <div className="space-y-2">
            ...
            {locs.map(row => {
              ...
              return (
                <div key={row.id} className={`rounded-xl border px-4 py-3 md:grid md:grid-cols-12 md:gap-3 flex flex-col gap-3 ${isApproved ? 'border-green-200 bg-green-50' : isError ? 'border-red-200 bg-red-50' : hasBanking ? 'border-amber-200 bg-amber-50/40' : 'border-gray-100 bg-white hover:border-gray-200'}`}>
                  {/* Col 1: Location info */}
                        ...
                        {!row.isManualMode ? (
                          ...
                          <div className="flex items-center gap-1 w-full">
                            <input type="text" placeholder="Routing" maxLength={9} className="w-[6rem] text-[11px] border border-gray-200 rounded-lg px-2 py-1.5 font-mono focus:outline-none focus:ring-1 focus:ring-blue-500"
                              onChange={(e) => updateLoc(row.id, { routingInput: e.target.value })} />
                            <input type="text" placeholder="Account" className="w-[7rem] text-[11px] border border-gray-200 rounded-lg px-2 py-1.5 font-mono focus:outline-none focus:ring-1 focus:ring-blue-500"
                              onChange={(e) => updateLoc(row.id, { accountInput: e.target.value })} />
                            <button onClick={async () => {
                              const bk = { routingNumber: row.routingInput || '', accountNumber: row.accountInput || '', accountNumberMasked: `••••${(row.accountInput || '').slice(-4)}`, authMethod: 'Manual' };
